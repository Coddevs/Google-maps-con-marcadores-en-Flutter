...
class MapSampleState extends State<MapSample> {
	...
	final _delayInPosition = Duration(seconds: 10); ///Retraso en actualizar la posición actual (My position)
	StreamSubscription<Position> _location; ///Stream que permite actualizar la posición actual (My position)
	LatLng _currentPosition; ///(My position)
	LatLng _targetPosition; ///Destino
	double _distanceInMeters;

	Future<void> _moveCamera({ ///Mueve la cámara (vista) del mapa hacia las coordenadas especificadas
	...
	Future<void> _moveMarker({ ///Mueve (o crea) el marcador dentro del mapa
		@required String markerId, ///Identificador del marcador
		@required LatLng position, ///Coordenadas del marcado
	}) async {
		...
		final positionMarker = Marker( ///Creando un marcador
			markerId: keyId,
			position: position,
		...
		_markers[keyId] = positionMarker; ///Asignando el marcador al mapa
		setState(() {});
	}
